...
/**
 * Contrôleur pour l'affichage du résultat de la simulation d'emprunt (dans un
 * tableau).
 * 
...
public class TableauAmortissementController {
				...
				switch (this.te) {
					case MOIS:
						...
						String infom = "Capital emprunté : " + this.montant + " | taux mensuel : " +
								df.format(tauxM) + "% | durée en mois : " + this.duree * 12 + " | Coût du prêt : "
								+ df.format((this.duree * 12) * montantRembourse) + "€";
						...
						montantpriColumn
								.setCellValueFactory(new PropertyValueFactory<LigneTableau, String>("montantpri"));

						TableColumn<LigneTableau, String> montantremboursementColumn = new TableColumn<>(
								"Montant à rembourser");
						montantremboursementColumn
								.setCellValueFactory(new PropertyValueFactory<LigneTableau, String>("montantrem"));
						...
						table.getItems()
								.add(new LigneTableau(periode, df.format(capDeb) + "€", df.format(interet) + "€",
										df.format(montantPri) + "€", df.format(montantRembourse) + "€",
										df.format(capFin) + "€"));

						for (int i = 2; i <= this.duree * 12; i++) {
							tableau(montantRembourse, this.capf, taux);
							table.getItems()
									.add(new LigneTableau(i, df.format(this.capd) + "€", df.format(this.interet) + "€",
											df.format(this.montantpri) + "€", df.format(montantRembourse) + "€",
											df.format(this.capf) + "€"));
						}
					...
					case ANNEE:
						...
						String infoA = "Capital emprunté : " + this.montant + "€" + " | Taux annuel : " +
								dfA.format(this.taux) + "% | Durée en année : " + this.duree + " | Coût du prêt :"
								+ dfA.format(this.duree * montantRembourseA) + "€";
						...
						TableColumn<LigneTableau, String> montantremboursementColumnA = new TableColumn<>(
								"Montant à rembourser");
						montantremboursementColumnA
								.setCellValueFactory(new PropertyValueFactory<LigneTableau, String>("montantrem"));
						...
						tableA.getItems()
								.add(new LigneTableau(periodeA, dfA.format(capDebA) + "€", dfA.format(interetA) + "€",
										dfA.format(montantPriA) + "€", dfA.format(montantRembourseA) + "€",
										dfA.format(capFinA) + "€"));

						for (int i = 2; i <= this.duree; i++) {
							tableau(montantRembourseA, this.capf, tauxA);
							tableA.getItems()
									.add(new LigneTableau(i, dfA.format(this.capd) + "€",
											dfA.format(this.interet) + "€", dfA.format(this.montantpri) + "€",
											dfA.format(montantRembourseA) + "€", dfA.format(this.capf) + "€"));
						}
			...
			case ASSURANCE:
				switch (this.te) {
					case MOIS:
						...
						String infom = "Capital emprunté : " + this.montant + "€" + " | taux mensuel : " +
								df.format(tauxM) + "% | durée en mois : " + this.duree * 12 + " | Coût du prêt : "
								+ df.format((this.duree * 12) * montantRembourse) + "€" + " | Taux assurance : "
								+ df.format(this.taux) + "%" + " | coût assurance : " + df.format(prixA) + "€"
								+ " | Coût Total : " + df.format(prixTota) + "€";
						this.info.setText(infom);
					...
					case ANNEE:
						...
						String infoA = "Capital emprunté : " + this.montant + "€" + " | Taux annuel : " +
								dfA.format(this.taux) + "% | Durée en année : " + this.duree + " | Coût du prêt : "
								+ dfA.format(this.duree * montantRembourseA) + "€" + " | Taux assurance : "
								+ dfA.format(this.taux) + "%" + " | coût assurance : " + dfA.format(prixAs) + "€"
								+ " | Coût Total : " + dfA.format(prixTot) + "€";
						;
						this.info.setText(infoA);
						this.info.setStyle("-fx-font-size : 15;");
						TableView<LigneTableau> tableA = new TableView<>();
	...
	/**
	 * Configure les différents éléments du tableau.
	 * 
	 * @param montantRembourse Le montant à rembourser
	 * @param capFin           Le capital final
	 * @param taux             Le taux de l'emprunt
	 * @author LAMOUR Evan
	 */
	public void tableau(double montantRembourse, double capFin, double taux) {
		this.capd = capFin;
		this.interet = this.capd * taux;
		this.montantpri = montantRembourse - this.interet;
	 ...
	 * @param ta               Le contrôleur de dialogue associé
	 * @param _dbstate         L'état courant de l'application
	 * @param montant          Le montant de l'emprunt
	 * @param taux             Le taux de l'emprunt
	 * @param tauxA            Le taux de l'assurance
	 * @param duree            La durée de l'emprunt
	 * @param te               Le type d'emprunt
	 * @param ts               Le type de simulation
	 * @author LAMOUR Evan
	 */
	public void initContext(Stage _containingStage, TableauAmortissement ta, DailyBankState _dbstate, int montant,
			double taux, double tauxA, int duree, TypeEmprunt te, TypeSimu ts) {
